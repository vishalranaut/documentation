...
## Profiling
Read-only tools for discovering, exploring, and analyzing [Continuous Profiler][60] data across services, runtimes, and traces. The profiling toolset lets you discover what is profiled (services, profile types, runtimes), filter and aggregate profiling data across services, drill from a trace or span into the corresponding flame graph, call graph, or timeline, and surface pre-computed service insights with recommended next steps.
...
*Toolset: **profiling***\
*Permissions Required: `Continuous Profiler Read`*\
Returns a timeline of lane groups (threads, GC, and so on) with CPU and I/O activity. Supports a critical-path mode (Go-only; requires traceContext) to identify latency bottlenecks within a span.
...
- Explore GC and CPU activity around the latency spike.
...
*Toolset: **profiling***\
*Permissions Required: `Continuous Profiler Read`*\
Returns values for a specific frame or context field discovered via get_profiling_fields. Sorted by frequency.
